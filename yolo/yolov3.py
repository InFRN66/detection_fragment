...
from .utils import convert_output_to_offset, targets_match_all, permute_sigmoid, decode


class DetectionLayer(nn.Module):
    def __init__(self, cfg, is_test):
        super(DetectionLayer, self).__init__()
        self.input_wh = cfg.image_size
    ...
    def forward(self, detections):
        ...
        all_detection = torch.cat(detections, 1) # large -> middle -> small [batch, all_anchors, (5+classes)]
...
class MultiboxLoss(nn.Module):
    ...
    def forward(self, detections, targets, debug=False):
        ...
        for key in self.all_anchors:
            anchor_list += self.all_anchors[key]
        ...
        for idx in range(batch_size):
            # match all targets
            targets_match_all(self.input_wh, self.pos_iou_threshold, targets[idx], decode_pred[idx][:, :4], anchor_list, self.grids,
